...
function radialeq(z,qQ)
    ...
    #println("zrad = ",Complex{Float64}(zrad))
    #println("thetas = ",Complex{Float64}(thrad[1]),",",Complex{Float64}(thrad[2]),",",Complex{Float64}(thrad[3]))
    #println("sigma = ",Complex{Float64}(sig0),"\t",",","s0 = ",Complex{Float64}(ess0))
    Acp = ArbNumerics.exp(-1im*pi*thrad[3]/2)*
          (ArbNumerics.cospi(thrad[1])-ArbNumerics.cospi(sig0)*ArbNumerics.cospi(thrad[2]))+
          ArbNumerics.exp(1im*pi*thrad[3]/2)*
          (ArbNumerics.cospi(thrad[2])-ArbNumerics.cospi(sig0)*ArbNumerics.cospi(thrad[1]))
    Acm = ArbNumerics.exp(1im*pi*thrad[3]/2)*
          (ArbNumerics.cospi(thrad[1])-ArbNumerics.cospi(sig0)*ArbNumerics.cospi(thrad[2]))+
          ArbNumerics.exp(-1im*pi*thrad[3]/2)*
          (ArbNumerics.cospi(thrad[2])-ArbNumerics.cospi(sig0)*ArbNumerics.cospi(thrad[1]))
    Bc = -1im*(ArbNumerics.cospi(thrad[1])-ArbNumerics.cospi(thrad[2]-sig0))*
         ArbNumerics.sinpi((thrad[3]-sig0)/2)*ArbNumerics.exp(1im*pi*sig0/2)
    Cc = -1im*(ArbNumerics.cospi(thrad[1])-ArbNumerics.cospi(thrad[2]+sig0))*
         ArbNumerics.sinpi((thrad[3]+sig0)/2)*ArbNumerics.exp(-1im*pi*sig0/2)
    Xp = (Acp - Bc*ArbNumerics.exp(-1im*pi*sig0)*ess0 - Cc*ArbNumerics.exp(1im*pi*sig0)/ess0)/
         ArbNumerics.sinpi(sig0)^2
    Xm = (Acm + Bc*ess0 + Cc/ess0)/ArbNumerics.sinpi(sig0)^2
    #println("Xm = ",Complex{Float64}(Xm),"\t",",","Cond = ", Complex{Float64}(ArbNumerics.exp(-1im*pi*(thrad[2]+thrad[3]/2))),",", "Diff=", Complex{Float64}(ArbNumerics.exp(-1im*pi*(thrad[2]+thrad[3]/2))-Xm))
...
eta = ArbNumerics.acos(0.999)

function rang(ini)
    i = ini
    arq = open("/home/joaocavalcante/Dropbox/Painlevé V/RN Black Hole/Subextremal RN/RNQnm_Subextremal.txt","w")
    zinitial = ArbComplex(0.0916203 - 1.30626im)
    while ( i < 0.002 )
	...
	println("qQ=",ArbFloat(qQ,digits=8),",","\t","omeg=",Complex{Float64}(solution[1][1]),",","\t","zrad=",Complex{Float64}(zrad)) 
...
function rang(ini)
    i = ini
    arq = open("/home/joaocavalcante/Dropbox/Painlevé V/RN Black Hole/Subextremal RN/s=0 l=0,1,2/RNQnm_Subextremal.txt","w")
    zinitial = ArbComplex(0.4497705669994054 - 0.004356913929723067im)
    while ( i < 0.9999999 )
        global  eta=ArbNumerics.acos(i)   # cos(eta)=Q/M; Need this as global
	solution = muller(fr_wrapper, zinitial, false )[1]
	
        thrad1 = zeros(ArbComplex,3)
        # z = M*omega
        # rplus/M = 1+sin(eta), rminus/M = 1-sin(eta)
        thrad1[1] = -spin-1im*(qQ-solution[1][1]*(1-ArbNumerics.sin(eta)))/ArbNumerics.sin(eta)+1im*(qQ-solution[1][1]*(1-ArbNumerics.sin(eta)))
        thrad1[2] = spin-1im*(qQ-solution[1][1]*(1+ArbNumerics.sin(eta)))/ArbNumerics.sin(eta)-1im*(qQ-solution[1][1]*(1+ArbNumerics.sin(eta)))
        thrad1[3] = -2*spin+4im*solution[1][1]-2*im*qQ
        zrad1 = 4im*(solution[1][1])*ArbNumerics.sin(eta)
        #println(zrad*thrad)
    
        lamb1 = (angularl-spin)*(angularl+spin+1)
        extrafac1 = lamb1+4*qQ*(solution[1][1])*(1+ArbNumerics.sin(eta))-4*(solution[1][1])^2*(1+ArbNumerics.sin(eta))^2-im*((1+2*spin)*(qQ-2*(solution[1][1]))+2*(solution[1][1])*ArbNumerics.sin(eta)) 

        function func2(x)
           return equation(thrad1,x,extrafac1,zrad1)
        end
        sig0 = muller( func2, ArbComplex(1.1-0.1im), false )[1]
...
#        sig0 = muller( func1, ArbComplex(1.1-0.1im), false )[1]
        ...
        println("Q/M=",ArbFloat(i,digits=8),",","\t","omeg=",Complex{Float64}(solution[1][1]),",","\t","L=",Complex{Float64}(thrad1[1]+thrad1[2]))   #,",","\t","sig0=",Complex{Float64}(sig0))
